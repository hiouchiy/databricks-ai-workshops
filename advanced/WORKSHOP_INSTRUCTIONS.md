...
チームで利用する場合、代表者がクイックスタートでリソースを作成した後、他のメンバーがアクセスできるように権限を付与してください。`workshop_setup.py` ノートブックの「チームメンバー権限」セルを使うか、以下を SQL エディタで実行します。

```sql
-- メンバーのメールアドレスを指定（複数人の場合は繰り返し実行）
GRANT USE CATALOG ON CATALOG `<CATALOG>` TO `member@company.com`;
GRANT USE SCHEMA ON SCHEMA `<CATALOG>`.`<SCHEMA>` TO `member@company.com`;
