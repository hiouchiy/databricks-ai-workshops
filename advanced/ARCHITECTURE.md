...
React は**ブラウザ上で動く JavaScript** です。サーバーではありません。Vite（ビルドツール）がソースコードをバンドルして、Express サーバーが配信する静的ファイル（HTML/CSS/JS）を生成します。
...
Express は **Python の FastAPI に相当する Node.js の Web サーバー** です。このアプリでは以下を担当します：

- **React UI の配信**: ビルド済みの HTML/CSS/JS をブラウザに返す
- **Databricks OAuth 認証**: ユーザーが誰かを識別する
- **チャット履歴**: Lakebase（PostgreSQL）に過去の会話を保存・取得
- **API 中継**: ブラウザからのチャットリクエストを FastAPI サーバーに転送
