# ワークショップ：Databricks で食品スーパー向け AI エージェントを構築する（改訂版）
...
| 7 | PyPI / npm アクセスの確認 | ローカル環境からパッケージインストールが可能であることを確認。必要に応じてプロキシ URL を参加者に共有 |
...
> **注意：** PyPI に直接接続できない環境の場合は、プロキシを設定してから `uv sync` を実行してください。
> ```bash
> UV_INDEX_URL=https://<your-pypi-proxy>/simple uv sync
> ```
...
| `<EXPERIMENT-ID>` | MLflow 実験の ID | `1159599289265540` |
...
`ja_execute_sql.py` を開き、先頭付近の `CATALOG` と `SCHEMA` を自分の値に書き換えます：
...
```bash
python3 ja_execute_sql.py --profile DEFAULT --warehouse-id <WAREHOUSE-ID>
```
...
`ja_execute_chunking.py` も同様に `CATALOG` と `SCHEMA` を書き換えて実行：

```bash
python3 ja_execute_chunking.py --profile DEFAULT --warehouse-id <WAREHOUSE-ID>
```
...
databricks experiments create-experiment "/Users/$DATABRICKS_USERNAME/retail-grocery-ltm-memory"
```
...
Databricks UI の **Experiments > retail-grocery-ltm-memory** でエージェントのトレースを確認できます。各リクエストの LLM 呼び出し、ツール実行、レイテンシなどが記録されています。
...
| `uv sync` で PyPI 接続エラー | `UV_INDEX_URL` にプロキシ URL を設定して再実行 |
