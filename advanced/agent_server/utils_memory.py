...
def memory_tools():
    ...
    @tool
    async def save_task_summary(task_title: str, summary_json: str, config: RunnableConfig) -> str:
        """Save a summary of a completed task to long-term memory. Call this after completing a discrete task for the user. The summary_json should be a JSON object with keys 'title' and 'summary'."""
        user_id = config.get("configurable", {}).get("user_id")
        if not user_id:
            return "Cannot save task summary - no user_id provided."

        store: Optional[BaseStore] = config.get("configurable", {}).get("store")
        if not store:
            return "Cannot save task summary - store not configured."

        thread_id = config.get("configurable", {}).get("thread_id", "unknown")
        namespace = ("task_summaries", user_id.replace(".", "-"))
        timestamp = datetime.now().isoformat()
        key = f"{thread_id}_{timestamp}"

        try:
            task_data = json.loads(summary_json)
            if not isinstance(task_data, dict):
                return f"Failed: summary_json must be a JSON object, not {type(task_data).__name__}"
            task_data["thread_id"] = thread_id
            task_data["timestamp"] = timestamp
            await store.aput(namespace, key, task_data)
            return f"Successfully saved task summary '{task_title}'."
        except json.JSONDecodeError as e:
            return f"Failed to save task summary: Invalid JSON - {e}"

    @tool
    async def search_task_history(query: str, config: RunnableConfig) -> str:
        """Search past task summaries from long-term memory. Use this when the user asks about specific tasks you helped them with previously."""
        user_id = config.get("configurable", {}).get("user_id")
        if not user_id:
            return "Task history not available - no user_id provided."

        store: Optional[BaseStore] = config.get("configurable", {}).get("store")
        if not store:
            return "Task history not available - store not configured."

        namespace = ("task_summaries", user_id.replace(".", "-"))
        results = await store.asearch(namespace, query=query, limit=5)

        if not results:
            return "No task history found for this user."

        items = []
        for item in results:
            title = item.value.get("title", "Untitled")
            summary = item.value.get("summary", "")
            timestamp = item.value.get("timestamp", "unknown")
            items.append(f"- [{timestamp}] **{title}**: {summary}")
        return f"Found {len(results)} past tasks:\n" + "\n".join(items)

    @tool
    async def save_conversation_summary(summary_json: str, config: RunnableConfig) -> str:
        """Save a summary of the current conversation to long-term memory. Call this when the user signals the conversation is ending (e.g., 'thank you', 'bye', 'that's all'). The summary_json should be a JSON object with keys 'summary' and 'topics' (list of topic strings)."""
        user_id = config.get("configurable", {}).get("user_id")
        if not user_id:
            return "Cannot save conversation summary - no user_id provided."

        store: Optional[BaseStore] = config.get("configurable", {}).get("store")
        if not store:
            return "Cannot save conversation summary - store not configured."

        thread_id = config.get("configurable", {}).get("thread_id", "unknown")
        namespace = ("conversation_summaries", user_id.replace(".", "-"))

        try:
            conv_data = json.loads(summary_json)
            if not isinstance(conv_data, dict):
                return f"Failed: summary_json must be a JSON object, not {type(conv_data).__name__}"
            conv_data["thread_id"] = thread_id
            conv_data["timestamp"] = datetime.now().isoformat()
            await store.aput(namespace, thread_id, conv_data)
            return "Successfully saved conversation summary."
        except json.JSONDecodeError as e:
            return f"Failed to save conversation summary: Invalid JSON - {e}"

    @tool
    async def search_past_conversations(query: str, config: RunnableConfig) -> str:
        """Search past conversation summaries from long-term memory. Use this when the user asks about previous conversations or what you've discussed before."""
        user_id = config.get("configurable", {}).get("user_id")
        if not user_id:
            return "Conversation history not available - no user_id provided."

        store: Optional[BaseStore] = config.get("configurable", {}).get("store")
        if not store:
            return "Conversation history not available - store not configured."
        ...
        for item in results:
            ...
            items.append(f"- [{timestamp}] Topics: {topics_str} — {summary}")
        return f"Found {len(results)} past conversations:\n" + "\n".join(items)

    return [get_user_memory, save_user_memory, delete_user_memory, save_task_summary, search_task_history, save_conversation_summary, search_past_conversations]
