**[日本語](#databricks-agent-chat-template)** | **[English](#databricks-agent-chat-template-1)**
...
このテンプレートは、Databricks 上にデプロイされたカスタムコードエージェントや Agent Bricks 向けの完全に動作するチャットアプリを提供しますが、他のユースケースについてはいくつかの[既知の制限事項](#既知の制限事項)があります。これらの制限事項への対応は現在進行中です。

## 機能

- **Databricks Agent およびFoundation Model 統合**: Databricks Agent Serving エンドポイントおよび Agent Bricks への直接接続
- **Databricks 認証**: Databricks 認証を使用してチャットアプリのエンドユーザーを識別し、会話を安全に管理します
- **永続的なチャット履歴（オプション）**: Databricks Lakebase（Postgres）を活用して会話を保存します。ガバナンスとレイクハウスとの緊密な統合が特徴です。データベースなしのエフェメラルモードでも動作可能です。
- **ユーザーフィードバック収集（オプション）**: アシスタントメッセージへの「いいね/よくないね」フィードバック機能。基盤となるトレースに MLflow アセスメントとして保存されます。MLflow Experiment リソースの設定が必要です。

## 前提条件

1. **Databricks Serving エンドポイント**: チャット対象の Agent Bricks またはカスタムエージェント Serving エンドポイントを含む Databricks ワークスペースへのアクセスが必要です。
2. **Databricks 認証の設定**
   - 最新バージョンの [Databricks CLI](https://docs.databricks.com/en/dev-tools/cli/install.html) をインストールしてください。macOS の場合は以下のコマンドで実行します：
   ```bash
   brew install databricks
   brew upgrade databricks && databricks -v
   ```
   - 以下のコマンドを実行して認証を設定します。
     以下のスニペットで `DATABRICKS_CONFIG_PROFILE` は認証設定に使用する Databricks CLI プロファイルの名前です。必要に応じて `dev_workspace` などお好みの名前に変更できます。
   ```bash
...
このプロジェクトには [Databricks Asset Bundle (DAB)](https://docs.databricks.com/aws/en/dev-tools/bundles/apps-tutorial) の設定が含まれており、必要なリソースの作成と管理を自動的に行うことでデプロイを簡素化します。
...
1. **リポジトリのクローン**:
   ...
   ```
2. **Databricks 認証**: [前提条件](#前提条件)に記載されている通り、認証が設定されていることを確認してください。
3. **Serving エンドポイントの指定と databricks.yml の TODO 対応**: `databricks.yml` 内の TODO を対応し、`serving_endpoint_name` のデフォルト値にチャット対象のカスタムコードエージェントまたは Agent Bricks エンドポイントの名前を設定します。オプションのコメントアウトされたセクションで以下を有効にできます：
   - **永続的なチャット履歴** — 2つのオプション `TODO` データベースブロックのコメントを解除して、Lakebase データベースのプロビジョニングとバインドを行います。詳細は[データベースモード](#データベースモード)を参照してください。**ヒント:** `./scripts/quickstart.sh` を実行すると自動的に設定されます。
   - **ユーザーフィードバック収集** — オプションの `TODO` experiment ブロックのコメントを解除し、experiment ID を設定します。データベースも必要です（両方のデータベース `TODO` ブロックのコメントを解除する必要があります）。詳細は[フィードバック収集](#フィードバック収集)を参照してください。**ヒント:** `./scripts/quickstart.sh` を実行するとデータベースとフィードバックの両方を自動的に設定できます。

   - 注意: [Agent Bricks Multi-Agent Supervisor](https://docs.databricks.com/aws/en/generative-ai/agent-bricks/multi-agent-supervisor) を使用する場合、MAS がオーケストレーションする基盤エージェントに対して、アプリのサービスプリンシパルに `CAN_QUERY` 権限を追加で付与する必要があります。これは `databricks.yml` にそれらのエージェント Serving エンドポイントをリソースとして追加することで行えます（`databricks.yml` 内の NOTE を参照してください）。
4. **バンドル設定の検証**:
...
5. **バンドルのデプロイ**。初回デプロイはリソースのプロビジョニング（特にデータベースが有効な場合）に数分かかることがありますが、以降のデプロイは高速です：
...
自動化されたクイックスタートスクリプトを使用すると、最も迅速にセットアップできます：
...
2. **クイックスタートスクリプトの実行**:
   ...
   - **前提条件のインストール** - jq、nvm、Node.js 20、Databricks CLI を自動的にインストール
   - **認証の設定** - Databricks CLI プロファイルの選択または作成を支援
   - **Serving エンドポイントの設定** - エンドポイント名の入力を求め、存在を検証
   - **データベースのセットアップ（オプション）** - 永続的なチャット履歴またはエフェメラルモードを選択
   - **Databricks へのデプロイ（オプション）** - リソースのデプロイとデータベースのプロビジョニングをオプションで実行
   - **ローカル環境の設定** - `.env` ファイルを自動的に作成・設定
   - **マイグレーションの実行** - データベースが有効な場合、データベーススキーマをセットアップ

   スクリプトは、データベースのプロビジョニング待機や接続情報の設定を含むセットアップ全体を自動的に処理します。
...
3. **アプリケーションの起動**:
   ...
   **ヒント:** `start-app.sh` スクリプトは、開発サーバー起動前に依存関係が最新であることを確認するため、初期セットアップ後のアプリ起動に便利です。
...
2. **環境変数の設定**:
   ...
   `.env` 内の TODO を対応し、Databricks CLI プロファイルとデータベース接続情報を指定してください。
...
ユーザーはアシスタントの応答に対して「いいね/よくないね」を付けることができます。フィードバックは基盤となるトレースに [MLflow アセスメント](https://docs.databricks.com/aws/en/generative-ai/agent-evaluation/assessments)として保存され、MLflow Experiment Tracking UI で簡単に確認・対応できます。

フィードバックは**デフォルトで無効**です。セットアップ手順は[フィードバック収集](#フィードバック収集)を参照してください。

> **注意:** 会話エージェントテンプレート（例：`agent-openai-agents-sdk`、`agent-langgraph`）を使用している場合、それらの `databricks.yml` はすでに MLflow experiment を作成・バインドしているため、`databricks bundle deploy` 後に追加設定なしでフィードバック機能が自動的に動作します。

### 永続的なチャット履歴

デフォルトでは、会話メッセージはメモリに保存され、サーバー再起動時に失われます。セッション間でチャット履歴を保持するには、`databricks.yml` で Lakebase データベースをバインドしてください。

セットアップ手順は[データベースモード](#データベースモード)を参照してください。
...
- チャットの会話が Postgres/Lakebase に保存されます
...
アプリケーションはデータベースなしでも動作します。このモードでは：

- チャットの会話は通常通り動作しますが、**保存されません**
- サイドバーに「No chat history available」と表示されます
- ヘッダーに小さな「Ephemeral」インジケーターが表示されます
- ユーザーは AI と会話できますが、ページ更新時に履歴が失われます
...
アプリはデータベース設定の有無を検出し、自動的に適切なモードで動作します。

#### インストール後のデータベース有効化

テンプレートをデータベースサポートなし（エフェメラルモード）でインストールし、後から永続的なチャット履歴を追加したい場合は、クイックスタートスクリプトを再実行できます：

```bash
./scripts/quickstart.sh
```

永続的なチャット履歴の有効化を求められたら「Yes」を選択してください。スクリプトは以下を行います：
- `databricks.yml` 内の必要なデータベースセクションのコメントを解除
- オプションで Lakebase データベースインスタンスをデプロイ
- `.env` ファイルにデータベース接続情報を設定
- データベースがプロビジョニングされている場合はマイグレーションを実行
- 正しいデータベース設定でローカル環境をセットアップ

スクリプトは以下を含むすべての設定を自動的に処理します：
- Databricks ワークスペースと認証の検出
- ターゲット環境に応じた正しいデータベースインスタンス名の算出
...
1. **`databricks.yml` の編集** - 2つのデータベースセクションのコメントを解除します：
...
チャットアプリはアシスタントメッセージへのオプションの「いいね/よくないね」フィードバックをサポートしています。有効にすると、フィードバックはエージェントエンドポイントが出力するトレースに [MLflow アセスメント](https://docs.databricks.com/aws/en/generative-ai/agent-framework/chat-app)として保存され、MLflow UI で簡単に確認・対応できます。

フィードバックは**デフォルトで無効**です。設定されていない場合、ヘッダーに「Feedback disabled」バッジが表示されます。

> **注意:** フィードバック投票の永続化（ページリロード時の「いいね/よくないね」状態の復元）にはデータベースが必要です。クイックスタートスクリプトを使用すると、両方の機能を一度に有効にできます。
...
スクリプトは自動的に以下を行います：
1. Serving エンドポイントに関連付けられた MLflow experiment ID を検索
2. `databricks.yml` 内のフィードバック `TODO` ブロックのコメントを解除して設定（`experiment_id` の設定）し、`app.yaml` の `MLFLOW_EXPERIMENT_ID` 環境変数を設定
3. `databricks.yml` 内の両方のデータベース `TODO` ブロックのコメントを解除して、Lakebase データベースのプロビジョニングとバインドを行う
...
**ステップ 1 — experiment ID の確認**

```bash
# カスタムコードエージェントまたは Agent Bricks Serving エンドポイントの場合
...
両方のデータベース `TODO` ブロック（投票永続化に必要）とフィードバック `TODO` ブロックのコメントを解除し、ステップ 1 で取得した experiment ID を設定します：
...
デプロイが完了すると、「Feedback disabled」バッジが消え、アシスタントメッセージの「いいね/よくないね」ボタンがアクティブになります。

### ローカル開発でのフィードバック有効化

`.env` ファイルにステップ 1 で取得した experiment ID を設定してください：
...
このプロジェクトは Playwright を使用したエンドツーエンドテストを採用しており、永続モードとエフェメラルモードの両方の動作を検証するデュアルモードテストをサポートしています。

### テストモード

テストは2つの異なるモードで実行され、データベースありとなしの両方の機能が正しく動作することを確認します：

#### データベースありモード

- データベース環境変数を使用（.env に設定されているか、別途宣言されている）
- 完全な Postgres データベースを使用
- チャット履歴の永続化、ページネーション、削除をテスト
- データベースが存在しない場合は警告を表示して停止

#### エフェメラルモード

- データベース接続なし（すべての POSTGRES_URL および PG* 変数を省略）
- 永続化なしでチャットストリーミングをテスト
- データベースがない場合の UI の適切な動作を確認

### テストの実行

**すべてのテストを実行（両モードを順次実行）**:

```bash
npm test
```

まずデータベースありモードのテストを実行し、次にエフェメラルモードのテストを実行します。モード間でサーバーは異なる設定で自動的に再起動されます。
...
GitHub Actions ワークフローは、2つのテストモードを別々のジョブで実行します：

- **test-with-db**: Postgres サービスを含み、マイグレーションを実行し、データベースありテストを実行
- **test-ephemeral**: Postgres なし、マイグレーションなし、エフェメラルテストを実行

両方のジョブは並列で実行され、CI フィードバックを高速化します。

## 既知の制限事項

- 画像やその他のマルチモーダル入力には対応していません
- Databricks で最も一般的で公式に推奨されている認証方法がサポートされています：ローカル開発向けの Databricks CLI 認証と、デプロイされたアプリ向けの Databricks サービスプリンシパル認証。その他の認証メカニズム（PAT、Azure MSI など）は現在サポートされていません。
- アプリコードがデータベースインスタンス内の固定スキーマ `ai_chatbot` を対象としているため、アプリごとに1つのデータベースを作成します。同じインスタンスから複数のアプリをホストするには：
  - `databricks.yml` でデータベースインスタンス名を更新
...
`databricks bundle` コマンド実行時に以下のようなエラー（または類似の「reference does not exist」エラー）が発生した場合、Databricks CLI のバージョンが古い可能性があります。
[前提条件](#前提条件)に従って最新バージョンの Databricks CLI をインストールし、再度お試しください。
...
これは、バンドル経由でデプロイされたリソースが手動で削除された場合や、バンドルで指定されたリソースが `databricks bundle` CLI を使用せずに手動で作成された場合に発生する可能性があります。この種の問題を解決するには、ワークスペース内の実際にデプロイされたリソースの状態を `databricks bundle summary` で確認し、バンドルの状態と比較してください。不一致がある場合は、[ドキュメント](https://docs.databricks.com/aws/en/dev-tools/bundles/faqs#can-i-port-existing-jobs-pipelines-dashboards-and-other-databricks-objects-into-my-bundle)を参照して、現在のバンドル状態からリソースを手動でバインド（リソースが手動で作成された場合）またはアンバインド（リソースが手動で削除された場合）してください。上記の例では、`chatbot_lakebase` データベースインスタンスリソースが `databricks bundle deploy` でデプロイされた後、手動で削除されました。これにより、バンドルの後続のデプロイが失敗しました（バンドルの状態はリソースが存在するはずだと示していましたが、ワークスペースには存在しなかったため）。`databricks bundle unbind chatbot_lakebase` を実行してインスタンスの削除をバンドル状態に反映させることで、`databricks bundle deploy` による後続のデプロイが可能になりました。
