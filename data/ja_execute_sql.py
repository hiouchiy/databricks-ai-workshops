...
Japanese localized version (日本語版):
  Domain data (names, addresses, products, stores) are localized to Japan.
  All code logic and structure remain identical to the original execute_sql.py.

Usage:
    python synthetic_data/ja_execute_sql.py --profile DEFAULT --warehouse-id <id>
"""
...
# ── Domain data (Japanese localized) ─────────────────────────────
FIRST_NAMES = [
...
PRODUCTS_BY_CATEGORY = {
    ...
    "お菓子": [
        ...
        ("ミックスナッツ 大容量", "稲葉", 898, "缶"), ("じゃがりこ", "カルビー", 158, "カップ"),
    ],
    "飲料": [
    ...
    "惣菜": [
        ...
        ("ポテトサラダ", "店内調理", 298, "パック"), ("ローストチキン", "店内調理", 598, "個"),
        ("マカロニサラダ", "店内調理", 248, "パック"), ("ひじき煮", "店内調理", 198, "パック"),
    ],
    "日用品": [
...
def main():
    ...
    for i, sname in enumerate(STORE_NAMES, 1):
        ...
        rows.append(f"('STORE-{i:02d}', '{esc(sname)}', '{random.choice(STREETS)}{random.randint(1,30)}-{random.randint(1,20)}-{random.randint(1,15)}', '{esc(city)}', '{state}', '{random.randint(100,999)}-{random.randint(1000,9999)}', '9:00～22:00', '{random_phone()}')")
